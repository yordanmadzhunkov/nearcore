...
use nearcore::config::{betanet_runtime_config, mainnet_genesis, testnet_runtime_config};
...
fn test_betanet_backwards_compatibility() {
    ...
    for protocol_version in [29u32, 34u32, 42u32, 50u32].iter() {
        ...
        let str = serde_json::to_string_pretty(&new_config)
            .expect("Failed serializing the runtime config");
        let mut file = File::create("/tmp/new_cfg.json").expect("Failed to create file");
        file.write_all(str.as_bytes()).unwrap();
...
#[test]
fn test_testnet_backwards_compatibility() {
    let genesis_runtime_config = testnet_runtime_config();
    let actual_runtime_config = ActualRuntimeConfig::new(genesis_runtime_config);
    ...
    for protocol_version in [29u32, 34u32, 42u32, 50u32].iter() {
        let mut old_config =
            actual_runtime_config.for_protocol_version(protocol_version.clone()).as_ref().clone();
        old_config.account_creation_config.min_allowed_top_level_account_length = 32;
        let new_config = store.get_config(protocol_version.clone()).as_ref().clone();
